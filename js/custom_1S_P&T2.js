...
window.onload = function() {
    // Always set autoBreakEnabled to 'word' (3W) when the page loads
    autoBreakEnabled = 'word';
    document.getElementById('autoBreakButton').innerText = '2W';
    
    // Update the logo with the forced break mode
    updateLogo();
};
...
window.onload = function () {
  ...
  const urlParams = new URLSearchParams(window.location.search);
  const logoId = urlParams.get('logoId'); // Used for logo text and settings
  const imageId = urlParams.get('logoId'); // Used for image URL and transformations
  ...
  // If imageId is present, load the image and transformations
  if (imageId) {
    ...
    get(imageRef).then((snapshot) => {
      if (snapshot.exists()) {
        ...
        // Set the image source and transformation values
        const imageContainer = document.getElementById('imageContainer');
        const imgElement = document.createElement('img');
        imgElement.src = logoData.url;

        imgElement.onload = () => {
          // Show share buttons and hide other elements
          const sharebuttons = document.getElementById('sharebuttons');
          const logosettings = document.getElementById('logosettings');
          const bottomNavbar = document.querySelector('.bottom-navbar');
          const imageLink = document.getElementById('imageLink'); // Make sure this element exists
          ...
          // New functionality you requested
          isImageLoaded = true;
          updateHeadingText();
          if (imageLink) {
        ...
        console.log("No image data found for this ID.");
      }
    });
  }
};
