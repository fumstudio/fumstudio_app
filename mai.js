...
async function loadCart(userId) {
    try {
        const cartRef = dbRef(db, `carts/${userId}/carts`);
...
async function handleLike(itemId, imageIndex) {
    ...
    try {
        ...
        // Perform all updates atomically with proper structure
...
function updateAllPrices(targetCurrency) {
    ...
    priceElements.forEach(element => {
        ...
        // Only update if the element is a price display (not percentage)
...
function monitorCurrencyChanges(userId) {
    ...
    onValue(userCurrencyRef, (snapshot) => {
        currentCurrency = snapshot.exists() ? snapshot.val() : 'ZAR';
        updateAllPrices(currentCurrency);
        
        // Update prices in the item data if it exists
        if (currentItemData) {
            renderItem(currentItemData);
        }
    }, (error) => {
...
async function renderItem(itemData) {
        ...
        itemData.images.forEach((imageGroup, index) => {
            ...
            html += `
                <div class="thumbnail-wrapper" data-index="${index}">
                    ...
                    ${itemsLeft > 0 ? `<div class="thumbnail-items-left">${itemsLeft} left</div>` : ''}
                </div>
            `;
        });
        html += `</div>`;
    ...
    if (itemData.images && itemData.images.length > 0) {
        ...
        html += `
            ...
            <div class="price-display" data-original-currency="ZAR">
                ${displayPrice}
                ${firstMainImage.discountPercentage > 0 && originalPrice ? `<span class="original-price" data-original-currency="ZAR">${originalPrice}</span><span class="discount-percentage">${firstMainImage.discountPercentage}% OFF</span>` : ''}
            </div>
        `;

        if (itemsLeft > 0) {
            html += `<div class="items-left-container"><span class="items-left">${itemsLeft} items left</span></div>`;
        }
            ...
            html += `
                <div class="sizes-container">
                    ...
                    <div class="sizes" id="sizes-container">
                        ${firstMainImage.sizes.map((size, i) => `<div class="size-option ${autoSelect && i === 0 ? 'selected' : ''}" data-size="${size}">${size}</div>`).join('')}
                    </div>
                </div>
            `;
        }
        ...
        html += `
            <div class="related-images-container">
                ...
                <div class="related-images">
                    ${itemData.images[0].relatedImages.map((img, index) => `<img src="${img.url}" class="related-image" data-index="${index}" alt="Related Image ${index + 1}" onerror="this.src='https://via.placeholder.com/300x375?text=No+Image';">`).join('')}
                </div>
            </div>
        `;
    }
    ...
    // Now properly define thumbnails after the HTML is inserted
    ...
    thumbnails.forEach(thumbnail => {
        thumbnail.addEventListener('click', function() {
            ...
            if (itemsLeftContainer) {
                itemsLeftContainer.innerHTML = selectedImage.itemsLeft > 0 ? `<span class="items-left">${selectedImage.itemsLeft} items left</span>` : '';
            }
...
function showAlert(message, type = 'info', autoClose = false) {
    ...
    iconContainer.innerHTML = `<i class="fas" style="color: #800080; font-size: 40px;">&#x${iconCode};</i>`;
...
function initializeSlider(itemData) {
    ...
    function goToSlide(index, instant = false) {
        if (!currentRelatedImages.length || isAnimating) return;
